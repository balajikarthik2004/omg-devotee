...
export function PaymentProcessor({
  templeObj, cause, frequency, pledgeDate, paymentMethod, setPaymentMethod, isProcessing, onPay, onCancel, getActiveAmount
}: any) {
  const { t: tStr } = useTranslation();

  const freqLabel = frequency === '15days' ? '15 days' : frequency === '30days' ? '30 days' : frequency.startsWith('custom-') ? `${frequency.split('-')[1]} days` : frequency;
  ...
  return (
    <div className="fade-in bg-card rounded-2xl shadow-sm overflow-hidden border border-border/50">
      <div className="bg-primary/5 border-b border-border p-5 md:p-6 flex items-center justify-between">
        <div>
          <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-1">{frequency === 'pledge' ? tStr("Amount to Pledge") : tStr("Amount to Pay")}</div>
          <div className="font-serif text-3xl font-bold text-foreground flex items-baseline gap-1">
            ...
            {frequency !== 'one-time' && (
              <span className="text-xs font-sans font-medium text-muted-foreground flex items-center gap-1 bg-background px-2 py-0.5 rounded-full border border-border/50">
                <RefreshCw className="w-3 h-3" /> / {freqLabel}
              </span>
            )}
          </div>
        </div>
      ...
      <div className="p-5 md:p-6">
        {frequency === 'pledge' ? (
          <div className="mb-6 p-4 bg-amber-50 rounded-xl border border-amber-200 text-center animate-in fade-in">
            <h2 className="font-bold text-[12px] uppercase tracking-widest text-amber-700 mb-2">{tStr("Pledge Confirmation")}</h2>
            <p className="text-sm font-medium text-amber-800">
              {tStr("You are promising to donate this amount by ")} <strong>{new Date(pledgeDate || "").toLocaleDateString()}</strong>.
              <br/><br/>
              {tStr("No payment is required right now. We will remind you when the date approaches.")}
            </p>
          </div>
        ) : (
          <>
        ...
        <div className="flex gap-2">
          <button onClick={onCancel} className="px-5 py-3 rounded-full bg-secondary/80 hover:bg-secondary text-white text-sm font-semibold transition-colors shadow-sm">
            {tStr("Back")}
          </button>
          <button onClick={onPay} disabled={isProcessing} className="flex-1 rounded-full bg-primary text-primary-foreground py-3 text-base font-bold flex items-center justify-center gap-2 transition-all hover:-translate-y-0.5 shadow-md shadow-primary/20 disabled:opacity-70 disabled:pointer-events-none active:scale-[0.98]">
            {isProcessing ? <Loader2 className="w-5 h-5 animate-spin" /> : frequency === 'pledge' ? tStr("Confirm Pledge") : `${frequency === 'one-time' ? tStr("Pay") : tStr("Subscribe")} ₹${getActiveAmount().toLocaleString()}`}
          </button>
        </div>
        {frequency !== 'one-time' && frequency !== 'pledge' && (
           <p className="text-center text-[10px] font-medium text-muted-foreground mt-3 leading-tight">
             {tStr("You will be charged")} ₹{getActiveAmount().toLocaleString()} {tStr("automatically every")} {freqLabel}. {tStr("You can cancel anytime.")}
           </p>
        )}
      </div>
    </div>
