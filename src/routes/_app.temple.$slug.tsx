...
function TempleDetail() {
  ...
  return (
    <div className="pb-32 lg:pb-8">
      {/* Hero */}
      <div className="relative h-[260px] lg:h-[340px] overflow-hidden" style={{ background: `linear-gradient(135deg, ${t.gradientFrom}, ${t.color})` }}>
        <svg viewBox="0 0 100 100" className="absolute -right-10 -bottom-10 w-72 h-72 text-white opacity-[0.08]"><text x="50" y="72" textAnchor="middle" fontSize="80" fill="currentColor" fontFamily="Noto Serif">ॐ</text></svg>
        <div className="absolute inset-x-0 top-0 flex items-center justify-between p-4">
          <Link to="/search" className="w-10 h-10 rounded-full bg-white/20 backdrop-blur flex items-center justify-center text-white"><ArrowLeft className="w-5 h-5" /></Link>
        ...
        <div className="absolute bottom-0 inset-x-0 p-5 lg:p-8 text-white">
          <h1 className="font-serif text-3xl lg:text-4xl font-semibold leading-tight drop-shadow">{t.name}</h1>
          <div className="text-sm opacity-90 mt-1">{t.established || "Ancient temple"} · {t.tier || "Major Kshetram"}</div>
          <div className="text-sm opacity-90 flex items-center gap-1 mt-1"><MapPin className="w-4 h-4" /> {t.city}, {t.district} District</div>
        </div>
      </div>
      ...
      <div className="sticky top-0 z-20 bg-white/95 backdrop-blur border-b border-border">
        <div className="max-w-5xl mx-auto px-4 lg:px-8 py-2.5 flex items-center gap-3 overflow-x-auto scrollbar-hide">
          ...
          <span className="text-sm flex items-center gap-1 text-foreground/80"><Clock className="w-4 h-4" /> {wait} min wait</span>
          <span className="text-sm text-foreground/80">🅿 {t.parking.lotA}% full</span>
          <span className="ml-auto text-xs text-muted-foreground hidden sm:inline">Updated just now</span>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 lg:px-8 py-6 grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* AI Recommendation */}
          <div className="bg-card border border-border rounded-2xl card-soft overflow-hidden">
            <div className="border-l-4 border-saffron p-5">
              <div className="flex items-center gap-2 text-saffron font-medium"><Sparkles className="w-5 h-5" /> AI Visit Recommendation</div>
              <div className="mt-3 text-sm text-muted-foreground">Best time to visit today:</div>
              ...
              <div className="mt-4 text-sm bg-secondary rounded-xl p-3 text-foreground/80">
              ...
              <Link to="/chat" className="mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-saffron">Ask AI a question →</Link>
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-5 card-soft">
            ...
            <Link to="/poojas" className="mt-3 inline-block text-sm font-medium text-saffron">View full pooja schedule →</Link>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-5 card-soft">
            ...
            <div className="pt-4 text-sm">
              {tab===0 && <div className="space-y-2 text-foreground/80"><div><span className="font-medium">Deity:</span> {t.deity}</div><div>{t.name} is one of Tamil Nadu's most revered temples. Devotees throng here especially on {t.specialDay}s. The temple is part of the Paadal Petra Sthalams tradition.</div></div>}
              {tab===1 && <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">{["♿ Wheelchair","🚻 Restrooms","🥛 Prasad","👟 Footwear","💊 First Aid","🚿 Bathing Ghat","📵 No Photography","🏧 ATM Nearby","🍽 Annadhanam","🅿 Free Parking"].map(f => <div key={f} className="bg-secondary rounded-xl p-2.5 text-sm text-center">{f}</div>)}</div>}
              ...
              {tab===3 && <div className="space-y-3"><div className="text-2xl font-serif font-semibold">4.3 <span className="text-sm text-muted-foreground">/ 5 · 2,841 reviews</span></div><div className="space-y-2"><div className="border border-border rounded-xl p-3"><div className="text-sm">"Very peaceful in the morning hours."</div><div className="text-xs text-muted-foreground mt-1">— Ramesh K · ⭐⭐⭐⭐⭐</div></div><div className="border border-border rounded-xl p-3"><div className="text-sm">"Queue was manageable at 4 PM."</div><div className="text-xs text-muted-foreground mt-1">— Priya S · ⭐⭐⭐⭐</div></div></div></div>}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {/* Crowd meter */}
          <div className="bg-card border border-border rounded-2xl p-5 card-soft">
            <h3 className="font-serif text-lg font-semibold">Live crowd</h3>
            <Gauge pct={Math.round(pct)} color={meterColor} />
            <div className="text-center text-sm text-muted-foreground -mt-2">{t.crowd.toLocaleString()} devotees inside</div>
            <div className="mt-4 grid grid-cols-3 gap-2 text-center">
              <Stat label="In queue" value="4,320" />
              <Stat label="Wait" value={`${wait}m`} />
              <Stat label="Darshan" value="8m" />
            </div>
            <button onClick={() => setAlertOn(a => !a)} className={`mt-4 w-full rounded-full py-2.5 text-sm font-medium border transition-colors ${alertOn?"bg-saffron text-white border-saffron":"bg-white border-border text-foreground hover:border-saffron/50"}`}>
...
function SlotLine({ ok, warn, bad, label, note }: any) {
  ...
  return <div className="flex items-center justify-between"><div className="flex items-center gap-2"><Icon className="w-4 h-4" style={{ color }} /><span>{label}</span></div><span className="text-xs text-muted-foreground">{note}</span></div>;
}
